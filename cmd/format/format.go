...
func Run(v *viper.Viper, statz *stats.Stats, cmd *cobra.Command, paths []string) error {
	...
	// open the db
	db, err := cache.Open(cfg.TreeRoot, cfg.NoCache)
	if err != nil {
		return fmt.Errorf("failed to open cache: %w", err)
	}

	// ensure db is closed after we're finished, and removed if temporary
	defer func() {
		path := db.Path()
	...
	for {
		...
		for idx := 0; idx < n; idx++ {
			...
			if file.CacheEntry == nil || file.CacheEntry.HasChanged(file.Info) {
				filesCh <- file
