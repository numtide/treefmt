...
func TestOnUnmatched(t *testing.T) {
	...
	_, err = execute(t, "-C", tempDir, "--allow-missing-formatter", "--on-unmatched", "fatal")
	...
	// default is warn
	out, err = execute(t, "-C", tempDir, "--allow-missing-formatter", "-c")
	as.NoError(err)
	checkOutput("WARN", out)

	out, err = execute(t, "-C", tempDir, "--allow-missing-formatter", "-c", "--on-unmatched", "warn")
	as.NoError(err)
	checkOutput("WARN", out)

	out, err = execute(t, "-C", tempDir, "--allow-missing-formatter", "-c", "-u", "error")
	as.NoError(err)
	checkOutput("ERRO", out)

	out, err = execute(t, "-C", tempDir, "--allow-missing-formatter", "-c", "-v", "--on-unmatched", "info")
	as.NoError(err)
	checkOutput("INFO", out)

	t.Setenv("TREEFMT_ON_UNMATCHED", "debug")
	out, err = execute(t, "-C", tempDir, "--allow-missing-formatter", "-c", "-vv")
	as.NoError(err)
...
func TestCpuProfile(t *testing.T) {
	...
	_, err = execute(t, "-C", tempDir, "--allow-missing-formatter", "--cpu-profile", "cpu.pprof")
	as.NoError(err)
	...
	_, err = execute(t, "-C", tempDir, "--allow-missing-formatter")
	as.NoError(err)
...
func TestAllowMissingFormatter(t *testing.T) {
	...
	_, err := execute(t, "--config-file", configPath, "--tree-root", tempDir)
	as.ErrorIs(err, format.ErrCommandNotFound)

	_, err = execute(t, "--config-file", configPath, "--tree-root", tempDir, "--allow-missing-formatter")
	as.NoError(err)
...
func TestSpecifyingFormatters(t *testing.T) {
	...
	_, err := execute(t, "-c", "--config-file", configPath, "--tree-root", tempDir)
	...
	_, err = execute(t, "-c", "--config-file", configPath, "--tree-root", tempDir, "--formatters", "elm,nix")
	...
	_, err = execute(t, "-c", "--config-file", configPath, "--tree-root", tempDir, "-f", "ruby,nix")
	...
	_, err = execute(t, "-c", "--config-file", configPath, "--tree-root", tempDir, "--formatters", "nix")
	...
	_, err = execute(t, "-c", "--config-file", configPath, "--tree-root", tempDir, "--formatters", "foo")
...
func TestPrjRootEnvVariable(t *testing.T) {
	...
	_, err := execute(t, "--config-file", configPath)
	as.NoError(err)
	assertStats(t, as, 32, 32, 32, 0)
}
...
func TestCache(t *testing.T) {
	...
	// clear cache
	_, err = execute(t, "--config-file", configPath, "--tree-root", tempDir, "-c")
	as.NoError(err)
	assertStats(t, as, 32, 32, 32, 0)
	...
	// no cache
	_, err = execute(t, "--config-file", configPath, "--tree-root", tempDir, "--no-cache")
	as.NoError(err)
	assertStats(t, as, 32, 32, 32, 0)
}
...
func TestFailOnChange(t *testing.T) {
	...
	test.WriteConfig(t, configPath, cfg)
	_, err := execute(t, "--fail-on-change", "--config-file", configPath, "--tree-root", tempDir)
...
func TestGitWorktree(t *testing.T) {
	...
	_, err = execute(t, "-c", "--config-file", configPath, "--tree-root", tempDir, "--walk", "filesystem")
	as.NoError(err)
	...
	_, err = execute(t, "-C", tempDir, "-c", "go", "-vv")
	as.NoError(err)
	assertStats(t, as, 2, 2, 2, 0)

	_, err = execute(t, "-C", tempDir, "-c", "go", "haskell")
	as.NoError(err)
	assertStats(t, as, 9, 9, 9, 0)

	_, err = execute(t, "-C", tempDir, "-c", "go", "haskell", "ruby")
	as.NoError(err)
	assertStats(t, as, 10, 10, 10, 0)

	// try with a bad path
	_, err = execute(t, "-C", tempDir, "-c", "haskell", "foo")
	...
	_, err = execute(t, "-C", tempDir, "-c", "foo.txt")
	as.NoError(err)
	assertStats(t, as, 0, 0, 0, 0)
}
...
func TestPathsArg(t *testing.T) {
	...
	_, err = execute(t, "-c", "elm/elm.json", "haskell/Nested/Foo.hs")
	as.NoError(err)
	assertStats(t, as, 2, 2, 2, 0)

	// specify a bad path
	_, err = execute(t, "-c", "elm/elm.json", "haskell/Nested/Bar.hs")
	...
	_, err = execute(t, "-c", externalPath)
...
func TestStdin(t *testing.T) {
	...
	out, err := execute(t, "-C", tempDir, "--allow-missing-formatter", "--stdin")
	...
	out, err = execute(t, "-C", tempDir, "--allow-missing-formatter", "test.nix")
	as.NoError(err)
...
`
	...
	out, err = execute(t, "-C", tempDir, "--allow-missing-formatter", "--stdin", "test.md")
	as.NoError(err)
	assertStats(t, as, 1, 1, 1, 1)
