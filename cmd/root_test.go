...
func TestGit(t *testing.T) {
	...
	// the .git folder contains 54 additional files
	// when added to the 30 we started with (31 minus nixpkgs.toml which we removed from the filesystem), we should
	// traverse 84 files.
	treefmt(t,
		...
		withStats(t, map[stats.Type]int{
			stats.Traversed: 84,
			stats.Matched:   84,
			stats.Formatted: 54, // the echo formatter should only be applied to the new files
			stats.Changed:   0,
		}),
	)
...
func TestJujutsu(t *testing.T) {
	...
	// the .jj folder contains 105 additional files
	// when added to the 30 we started with (31 minus nixpkgs.toml which we removed from the filesystem), we should
	// traverse 135 files.
	treefmt(t,
		withArgs("--walk", "filesystem"),
		...
		withStats(t, map[stats.Type]int{
			stats.Traversed: 135,
			stats.Matched:   135,
			stats.Formatted: 105, // the echo formatter should only be applied to the new files
			stats.Changed:   0,
		}),
	)
